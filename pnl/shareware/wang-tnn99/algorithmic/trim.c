...
/* It takes the PITCHMASK produced earlier, does segmentation, and         */
/* performs conformation and trimming by the longest segment.              */
/* Its output will be read by the program "stream.c", and the              */
/* latter produces the outcome of speech segregation (two streams).        */
/***************************************************************************/
...
short pitchmask[MAX_FRAME][MAX_CHANNEL];
...
void update(int s, point *p)
{
	int frame, chan;

	if (p!=NULL) {
		if (segment[s].oneCount[p->frame]>segment[s].twoCount[p->frame])
			pitchmask[p->frame][p->chan]=1;
		else
			pitchmask[p->frame][p->chan]=2;
		update(s,p->next);
		}
}
...
main(int argc, char **argv)
{
	...
	for (frame=0; frame<numFrames; frame++)
		for (chan=0; chan<numChannels; chan++) {
			scanf("%d",&dummy);
			channels[frame][chan]=pitchmask[frame][chan]=(short)dummy;
			}
	...
	/* TEMPORARY CODE */
	/* create a pitch mask with
	/* trim the pitch mask */
/*
	for (s=0; s<numSegment; s++) 
		update(s,segment[s].head);
*/

	for (frame=0; frame<segment[seed].first; frame++)
		for (chan=0; chan<numChannels; chan++)
			pitchmask[frame][chan]=0;

	for (frame=segment[seed].last+1; frame<numFrames; frame++)
		for (chan=0; chan<numChannels; chan++)
			pitchmask[frame][chan]=0;
	...
	for (frame=0; frame<numFrames; frame++) {
		for (chan=0; chan<numChannels; chan++)
			printf("%1d ",pitchmask[frame][chan]);
		printf("\n");
		}
}
