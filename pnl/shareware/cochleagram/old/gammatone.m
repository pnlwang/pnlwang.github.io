...
sig = reshape(in,1,sigLength);      % convert input to row vector

% gammatone filtering using FFT
d = 2^ceil(log2(sigLength + gL));   % fft signal length with zero padding
r = real(ifft((fft(repmat(sig,numChan,1)',d).*fft(gt',d))))';
r = r(:,1:sigLength);
